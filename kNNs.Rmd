...
findKNNSingle <- function(dists, classes){
    
}

lapply(1:10, function(k){
    k_class1 <- S$Class[1:k] %>% droplevels() %>% table() %>% .[1]
    k_class2 <- S$Class[1:k] %>% droplevels() %>% table() %>% .[2]
    k_prop <- k_class1/k_class2
    if(k_prop > 0.5){
        k_pred <- names(k_class1)
    } else if(k_prop < 0.5){
        k_pred <- names(k_class2)
    } else { k_pred <- "Tie" }

    data.frame(k.Neighbors = k, Predicted.Class = k_pred, 
               Predicted.Proportion = k_pred_prop)
}) %>% bind_rows()
